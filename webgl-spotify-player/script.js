...
function initKeyboard() {
  window.addEventListener('keyup', function (event) {
    ...
    if (event.keyCode == 49) {
      sendPlayContext('spotify:album:2gaw3G7HBQuz93N8X89JIA', 1);
    }
    if (event.keyCode == 50) {
      sendPlayContext('spotify:album:2KWlNb50pLNM11pGqqVdSX');
    }
    if (event.keyCode == 51) {
      sendPlayContext('spotify:album:7xrc6SpiFhcgBaLYbqfB7k', 1);
    }
    if (event.keyCode == 52) {
      sendPlayContext('spotify:album:64XdBdXNdguPHzBg8bdk5A');
    }
    if (event.keyCode == 53) {
      sendPlayContext('spotify:album:64XidJaSHIS1XMb4Po77b1', 9);
    }
    if (event.keyCode == 54) {
      sendPlayContext('spotify:album:4wJmWEuo2ezowJeJVdQWYS', 1);
    }
    if (event.keyCode == 55) {
      sendPlayContext('spotify:album:5uTGqtnYpSRYiFTEuQcmNE', 0);
    }
    if (event.keyCode == 56) {
      sendPlayContext('spotify:album:4QNlqYSMYCPiKZfzUfH7jK', 1);
    }
    if (event.keyCode == 57) {
      sendPlayContext('spotify:album:29JfxOC3yMXwy3KlX8WFUQ', 1);
    }
    if (event.keyCode == 48) {
      sendPlayContext('spotify:album:68zh8sbZPMeJb7GnqomRJS', 0);
    }
